...
修改模板或 Prompt 时，再阅读 `templates/project/` 与 `templates/prompts/` 中的对应文件；接入独立 AI 执行层时同时阅读 `templates/prompts/delegate-task.md`。

## 维护规则

- Task Board 核心协议保持工具无关；AgentDock 是教程采用的电脑 MCP 运行时，不是项目状态来源。
- Task Board 保存项目级状态、Task Revision、Claim 与 Complexity/Risk 等路由输入，不保存具体物理模型绑定或 attempt history；Backend/Model/Effort/attempt 属于下游执行工作流。
...
- `templates/prompts/scheduled-developer.md` 只依赖项目中可以被明确找到的状态，不依赖聊天记忆；READY→IN_PROGRESS 领取必须使用 `tools/coordinator_lock.py` 的短时原子锁，不能只靠“写完后再读一次”避免竞争。
